...
class ApiController < ApplicationController
    ...
    def new_message_to_user
        if params[:user_id] != nil and
           params[:to_user] != nil and
           params[:message] != nil

        		@user = User.find(params[:user_id])
        		@to_user = User.find_by_user_name(params[:to_user])
        		if @to_user != nil
	...
	def get_num_of_messages
		if params[:user_id] != nil
			user = User.find(params[:user_id])
			render :json => "{\"count\":\"#{user.messages.where(:status => 0).count}\"}"
		else
			render :json => '{"message":"error in params"}'
		end	
	end
